...
import { CFG, QUALITY, RINGS, DENS_POW } from './config.js';
import { LIN, C } from './palette.js';
import { NO_CAST, addMesh, addBulk, setDepth } from './scene-contract.js';
import { TAU, DEG, clamp, lerp, smoothstep, smootherstep, rng, noise2, fbm2, ridged, billow } from './math.js';

/*═══════════════════════════════════════════════════════════════════════════════
  HOSHI-NO-TANI  ·  a procedural Ghibli valley
  ─────────────────────────────────────────────────────────────────────────────
  Extracted so far, and imported above:
      config.js    §0   world constants, quality presets, grass rings
      palette.js   §0b  every colour in the film, sRGB -> linear
      scene-contract.js §0c  addMesh / addBulk / setDepth — READ THIS FIRST
      math.js      §1   noise, fbm, deterministic PRNG

  Still in this file, and next to be carved out:
      §2  glsl library              §9  village + viaduct
      §3  terrain                   §10 train + smoke
  ...
  (There is no §11.  The original table of contents advertised one, listing
   pollen, birds and butterflies; the particles live at the end of §10, the
   mote and bird logic sits in §15, and there are no butterflies anywhere.)

  The error handler that used to open this file now lives in index.html as a
  classic script, so it is registered before any module evaluates.
═══════════════════════════════════════════════════════════════════════════════*/
...
const SOLIDS = [];
function pushSolid(x, z, hx, hz, yaw){
