...
**The fastest way:** download or clone the repo and double-click
**`dist/index.html`**. That's it — no install, no build, no server. It is a
single self-contained file and it opens straight from your filesystem.

You will need an internet connection the first time, because Three.js is loaded
from a CDN. Give it a few seconds to build the world (it is carving a
terrain, growing a meadow and stacking clouds while you wait), then click
**Enter the valley**.
